            ...
            $(document).ready(function () {
                ...
                $("#ddlProjName").append('<option selected>Select Project</option>');

                $("#ddlProjName").change(function () {
                    var selectedProj = $(this).children("option:selected").val();
                    $("#txtProjURL").attr('readonly', false);
                ...
                $("#btnStart").click(function () {
                    var f = $("#csvFile").val();
                    var len = $("#csvFile").val().length;
                    var csv = $("#csvFile").val().substr(len - 3);
                    //alert(csv);
                    ...
                    if ($("#txtProjURL").val().length === 0) {
                        ...
                        $("#msgHtml").html("<table border=0><tr><td><img src='../Images/alert.png'/></td><td>" + " File Path Required" + "</td></tr></table>");
                        return false;
                    } else if (csv !== "csv") {
                        $("#msgHtml").html("<table border=0><tr><td><img src='../Images/alert.png'/></td><td>" + " File must be CSV" + "</td></tr></table>");
                        return false;
                    } else {
                        $("#msgHtml").html("");
                        return true;
...
function formSetup() {
    ...
    $("#rbAction_0").attr('disabled', true);
    $("#rbAction_1").attr('disabled', true);
    $("#rbAction_2").attr('disabled', true);
    $("#rbAction_3").attr('disabled', true);
    $("#rbAction_4").attr('disabled', true);
    $("#ddlProjName").attr('disabled', true);
    $("#txtProjURL").attr('disabled', true);
            ...
            function confirmDelete() {
                ...
                var r = confirm("Delete ALL items in this list?");
                if (r === true) {
            ...
            function popup(url) {
                popupWindow = window.open(url, 'popUpWindow', 'height=600,width=700,left=10,top=100,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no, status=yes');
            }
            ...
            function cancelPostBack(sender, args) {
                if (Sys.WebForms.PageRequestManager.getInstance().get_isInAsyncPostBack()) {
                    alert('Current process stil runniing');
            ...
            function renderChrome() {
                ...
                var options = {
                    ...
                    "appTitle": "CSV Uploader For SharePoint"
                };
